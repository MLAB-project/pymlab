...
class DriverRegistry(object):
    ...
    def __pprint__(self, printer, level = 0):
        ...
        printer.indent()
        pairs = list(self.drivers.iteritems())
        l = max((len(k) for k, v in pairs))
        for k, v in pairs[:-1]:
            printer.writef("{}  {}.{}\n", k.ljust(l), v.__module__, v.__name__)
        if len(pairs) > 0:
            k, v = pairs[-1]
            printer.writef("{}  {}.{}", k.ljust(l), v.__module__, v.__name__)
        printer.unindent()
    
    def add_class(self, names, cls):
        if isinstance(names, basestring):
            names = [names, ]
        
        LOGGER.info("Driver registered: class = %s, names = %r.", cls.__name__, names)
        for name in names:
...
def main():
    ...
    from pymlab.utils import PrettyPrinter
    pp = PrettyPrinter()
    pp.format(DRIVER_REGISTRY)
    pp.writeln()
