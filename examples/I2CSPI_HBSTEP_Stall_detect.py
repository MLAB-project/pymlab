...
try:
    ...
    spi.SPI_write(spi.I2CSPI_SS0, [0xC0])

#StallTh_to_Par(StallTh) ((uint8_t)(((StallTh - 31.25)/31.25)+0.5))  # Stall Threshold conversion, range 31.25mA to 4000mA */
    ...
    while True:
        ...
        spi.SPI_write(spi.I2CSPI_SS0, [0x40])       ## run to defined step position
        spi.SPI_write(spi.I2CSPI_SS0, [0x00])
        ...
        for x in range(100):
            ...
            data = spi.SPI_read(1)
            stall = (~(data[0] >> 3)) & 0xF
            spi.GPIO_write(stall)
            spi.SPI_write(spi.I2CSPI_SS0, [0x00])

            data.extend(spi.SPI_read(1))
            print map(hex, data)
            if stall > 0 :
                spi.SPI_write(spi.I2CSPI_SS0, [0xB8])
